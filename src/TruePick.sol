...
contract TruePick is VRFConsumerBaseV2Plus, AutomationCompatibleInterface {
    ...
    error TruePick__YouLostBetterLuckNextTime(
        uint256 resultValue,
        uint256 userGussed
    );
    ...
    function performUpkeep(bytes calldata) external override {
        ...
        if (!upkeepNeeded) {
            revert TruePick__UpkeepNotRequired(
    ...
    function getWinner() public {
        if (s_gameState != GameState.ENDED) {
            revert TruePick__GameNotEnded();
        }
        if (s_playerStateTrack[msg.sender] != PlayerState.ENTERED) {
            revert TruePick__UserIsNotPlayer();
        }
        if (s_playerGussed[msg.sender] != s_resultValue) {
            revert TruePick__YouLostBetterLuckNextTime(
                s_resultValue,
                s_playerGussed[msg.sender]
            );
        }
    ...
    function getGuessingRange() public view returns (uint256) {
        return s_guessingRange;
    }

    function getCurrentGameOperator() public view returns (address) {
        return s_gameOperator;
    }

    function getPlayerGuessed(address _player) public view returns (uint256) {
        return s_playerGussed[_player];
    }

    function getPlayerState(address _player) public view returns (uint256) {
